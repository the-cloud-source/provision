...
network --device eth0 --bootproto dhcp --hostname blipper.porto.local
...
logvol /var/lib/mysql --fstype ext4 --name=lvmysql --vgname=vgroot --size=8192

services --enabled  ntpd,httpd,mysqld,rsyslogd
...
#repo	--name=install-updates         --mirrorlist=http://mirrorlist.centos.org/?release=5&arch=x86_64&repo=updates
 repo	--name=install-updates         --baseurl=http://mirrors.nfsi.pt/CentOS/5.11/updates/x86_64/
